...
**Additional packages** extend R with new functions
...
A **collection of R packages** for data science, designed to work together.

These are the main packages from the tidyverse that we will use:<br><br>

:::{.r-stack}

![](img/hex-stickers/tibble.png){width=15%}![](img/hex-stickers/readr.png){width=15%}![](img/hex-stickers/readxl.png){width=15%}![](img/hex-stickers/tidyr.png){width=15%}![](img/hex-stickers/ggplot2.png){width=15%}![](img/hex-stickers/dplyr.png){width=15%}
...
Then load the packages at the beginning of your script:
...
- Tibbles are part of the `tibble` package (included in the tidyverse)
...
#| eval: false
library(tidyverse)
# library(tibble) # also works if you only need tibbles
...
- are pretty similar, however tibbles are slightly more convenient and modern
...
#### packages and tibbles

- additional packages extend R with new functions
- install once with `install.packages("tidyverse")`, load with `library(tidyverse)`
- tibbles are part of the tidyverse (or load individually with `library(tibble)`)
