...
```{r echo=FALSE}
print(select(and_vertebrates, starts_with("s")),n=3)
```
...
```{r echo=FALSE}
print(select(and_vertebrates, year:unittype), n=3)
```
...
```{r echo=FALSE}
print(mutate(and_vertebrates, weight_kg = weight_g/1000), n= 3)
```
...
```{r echo=FALSE}
species <- select(and_vertebrates, species) |> distinct() |> 
...
`left_join()` means that the resulting tibble will contain all rows of `and_vertebrates`,
