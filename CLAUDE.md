...
Solutions are revealed automatically via `timer_solution.js`. The setup is:

- **`timer_solution.js`** (project root, copied to `docs/` as a resource): contains an array of session end datetimes, one per course day
- **Each session QMD** with a solution sets `const course_day = N` inline and loads the script — these never need to change between course runs
- When the current time passes `session_end_times[course_day - 1]`, the solution link (hidden with `style="display:none"`) becomes visible

**For a new course run:** edit only `docs/timer_solution.js` with the new session dates and push — no re-render needed.

**To add a new session with a solution**, follow the pattern in any existing session QMD (e.g. `sessions/02_intro-r.qmd`).
