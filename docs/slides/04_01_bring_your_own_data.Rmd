...
- Site ID (`SITE`)
- Bird abundance in the fragment (`ABUND`)
- Area (size of the fragment) (`AREA`)
- Distance to the next forest fragment (`DIST`)
- Distance to the next large forest fragment (`LDIST`)
- Years since site was isolated (`YR.ISOL`)
- Grazing intensity of the surrounding (`GRAZE`)
- Altitude (`ALT`)
...
- Explore the effects of environment on bird abundance (try models or tests)
]
...
- [janitor package](https://garthtarr.github.io/meatR/janitor.html): some simple functions for examining and cleaning dirty data
...
- [Join multiple tables](https://dplyr.tidyverse.org/articles/two-table.html): `dplyr` offers function to join multiple tables based on a common column
