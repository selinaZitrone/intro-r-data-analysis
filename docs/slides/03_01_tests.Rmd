...
```{r eval=FALSE}
select(mydata, normal)
```

```{r echo=FALSE}
print(select(mydata, normal),n=3)
```

--

This is important because most tests work with vectors as input.
...
wilcoxon.test(treatA, treatB, paired = TRUE)
