...
#1c. Binomial distribution varying "pi" ----
binom_list <- list()
...
for (i in 1:length(p_vec)){
  binom_list[[i]] <- data.frame(x = x,
                                Density = dbinom(x, p = p_vec[i], size = N),
                                p = p_vec[i]
  )
}
...
ggsave("./slides/img/day03/binom_plot.png",binom_plot, width = 16, height = 9, unit = "cm")
...
ggsave("./slides/img/day03/poiss_plot.png",poiss_plot, width = 16, height = 9, unit = "cm")
