...
# Linear regression in `r fa("r-project")`
...
```{r echo=FALSE, fig.height=6.1}
 ggplot(prod, aes(x=richness, y=productivity, color= site))+
   geom_point() +
   scale_color_manual(values = c("#4C7488", "#D78974")) +
   geom_abline(slope = slope, intercept = intercept1 , color = "#D78974")+#<<
   geom_abline(slope = slope, intercept = intercept2 , color = "#4C7488")#<<
```
]
...
- The aestethetic mapping is inherited from the top level `ggplot` call to the `geom_line`
