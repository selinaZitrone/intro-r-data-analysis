...
The data:
  - random sample of n data points ( $Y_i, X_{i1}, X_{i2},..., X_{ip}$, $i = 1...n$) 
    - $X_{i1}, X_{i2},..., X_{ip}$ are independent **predictor** variables
...
![](img/day3/lm_residuals_placeholder.png)
---
# Assumptions for linear models

.pull-left[

Finding the parameters that lead to the lowest RSS works only if:

- residuals are normally distributed
- residual variance is constand
- no strong outliers

This has to be checked for every model!
]
...
Lets start with a Hypothesis:

.pull-left[
 $H_0$: Productivity increases with species richness.
...
We can use the function `lm` to fit a linear model. The general structure of the function call is like this:

.center[<b>lm ( formula = .col1[Y] ~ .col2[X], data = .col3[dat] )</b>]

with

- .col1[Y] being the response variable
- .col2[X] being the predictor(s)
- .col3[dat] being the name of the data frame

Multiple predictors can be added with `+`, `:` or `*` depending on interaction:

- <b>`Y ~ X1 + X2`</b>: test effect without interaction
- <b>`Y ~ X1 + X2 + X1:X2`</b>: test single effects and interaction betwenn `X1` and `X2`
- <b>`Y ~ X1*X2`</b>: short for testing all single effects and all interactions (here equivalent to line above)
...
<b>lm ( formula = .col1[Y] ~ .col2[X], data = .col3[dat] )</b>
...
```{r echo=FALSE}
  ...
  geom_point() +
  theme(
    axis.title.y = element_text(color = "#F3BC50", face = "bold"),
    axis.title.x = element_text(color = "#25C0C0", face = "bold")
  )
```
]
...
<b>$H_0$:</b> The error variance in the simple model is not significantly higher than in the more complex model
...
![:scale 70%](img/day3/anova_explain_placeholder.png)
---
...
Placeholder see day 5 slide 31-32 and 50 for explanation
